...
var Registry = /** @class */ (function () {
    ...
    Registry.prototype.list = function (ids) {
        ...
        return __spread(this.ordered); // copy of everythign just in case
    };
...
function setMarkdownOptions(optionsOverride) {
    marked.setOptions(__assign({}, defaultMarkedOptions, optionsOverride));
}
...
var CircularVector = /** @class */ (function () {
    function CircularVector(buffer) {
        this.length = buffer.length;
        this.buffer = buffer;
        this.index = 0;
    }
    CircularVector.prototype.append = function (value) {
...
var DataFrameHelper = /** @class */ (function () {
    function DataFrameHelper(data) {
        this.length = 0; // updated so it is the length of all fields
        this.fieldByName = {};
        this.fieldByType = {};
        if (!data) {
            data = { fields: [] }; //
    ...
    DataFrameHelper.prototype.addFieldFor = function (value, name) {
        if (!name) {
     ...
     */
    DataFrameHelper.prototype.reverse = function () {
        var e_1, _a;
        try {
            for (var _b = __values(this.fields), _c = _b.next(); !_c.done; _c = _b.next()) {
                var f = _c.value;
                if (isArray_1(f.values)) {
                    var arr = f.values;
                    arr.reverse();
                }
            }
    ...
    DataFrameHelper.prototype.updateTypeIndex = function (field) {
    ...
    DataFrameHelper.prototype.addField = function (f) {
        var e_2, _a;
        var type = f.type || exports.FieldType.other;
        var values = !f.values || isArray_1(f.values)
            ? new ArrayVector(f.values) // array or empty
            : f.values;
        // And a name
        var name = f.name;
        if (!name) {
            if (type === exports.FieldType.time) {
     ...
     */
    DataFrameHelper.prototype.appendRow = function (row) {
     ...
     */
    DataFrameHelper.prototype.appendRowFrom = function (obj) {
        var e_3, _a;
        try {
            for (var _b = __values(this.fields), _c = _b.next(); !_c.done; _c = _b.next()) {
                var f = _c.value;
    ...
    DataFrameHelper.prototype.getFields = function (type) {
        if (!type) {
    ...
    DataFrameHelper.prototype.hasFieldOfType = function (type) {
        var types = this.fieldByType[type];
        return types && types.length > 0;
    };
    DataFrameHelper.prototype.getFirstFieldOfType = function (type) {
    ...
    DataFrameHelper.prototype.hasFieldNamed = function (name) {
        return !!this.fieldByName[name];
    };
    /**
     * Returns the first field with the given name.
     */
    DataFrameHelper.prototype.getFieldByName = function (name) {
        return this.fieldByName[name];
    };
...
function convertTableToDataFrame(table) {
    ...
    var fields = table.columns.map(function (c) {
        var text = c.text, disp = __rest(c, ["text"]);
        return {
            name: text,
            config: (disp || {}),
...
var guessFieldTypes = function (series) {
    for (var i = 0; i < series.fields.length; i++) {
        if (!series.fields[i].type) {
            ...
            return __assign({}, series, { fields: series.fields.map(function (field) {
                    if (field.type && field.type !== exports.FieldType.other) {
                        return field;
                    }
                    // Calculate a reasonable schema value
                    return __assign({}, field, { type: guessFieldTypeForField(field) || exports.FieldType.other });
                }) });
        }
    }
...
var toDataFrame = function (data) {
    if (data.hasOwnProperty('fields')) {
        ...
        if (data.hasOwnProperty('rows')) {
            var v = new DataFrameHelper(data);
            var rows = data.rows;
        ...
        return new DataFrameHelper(data);
    }
...
var toLegacyResponseData = function (frame) {
    ...
    if (fields.length === 2) {
        ...
        if (!type) {
            type = guessFieldTypeForField(fields[1]) || exports.FieldType.other;
        }
        if (type === exports.FieldType.time) {
            return {
...
function sortDataFrame(data, sortIndex, reverse) {
    ...
    // String Comparison
    if (field.type === exports.FieldType.string) {
    ...
    return __assign({}, data, { fields: data.fields.map(function (f) {
            return __assign({}, f, { values: new SortedVector(f.values, index) });
        }) });
}
/**
...
var CSVReader = /** @class */ (function () {
                        ...
                        if (idx > 0) {
                            ...
                            if (isName || headerKeys.hasOwnProperty(k)) {
                                ...
                                if (_this.state === ParseState.ReadingRows) {
                                    _this.current = new DataFrameHelper({ fields: [] });
    ...
    CSVReader.prototype.readCSV = function (text) {
        ...
        var papacfg = __assign({}, this.config, { dynamicTyping: false, skipEmptyLines: true, comments: false, step: this.step });
...
function reduceField(options) {
    ...
    if (data.length < 1) {
        var calcs = __assign({}, field.calcs);
        try {
    ...
    if (queue.length === 1 && queue[0].reduce) {
        ...
        field.calcs = __assign({}, field.calcs, values_1);
        return values_1;
    }
    ...
    try {
        for (var queue_2 = __values(queue), queue_2_1 = queue_2.next(); !queue_2_1.done; queue_2_1 = queue_2.next()) {
            ...
            if (!values.hasOwnProperty(reducer.id) && reducer.reduce) {
                values = __assign({}, values, reducer.reduce(field, ignoreNulls, nullAsZero));
            }
        }
    }
    catch (e_3_1) { e_3 = { error: e_3_1 }; }
    ...
    field.calcs = __assign({}, field.calcs, values);
    return values;
}
// ------------------------------------------------------------------------------
//
...
function doStandardCalcs(field, ignoreNulls, nullAsZero) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (currentValue !== null) {
            var isFirst = calcs.firstNotNull === undefined;
            if (isFirst) {
...
function getLogLevel(line) {
    ...
    try {
        for (var _b = __values(Object.keys(exports.LogLevel)), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            if (regexp.test(line)) {
                var level = exports.LogLevel[key];
                if (level) {
                    return level;
                }
            }
...
function addLogLevelToSeries(series, lineIndex) {
    ...
    return __assign({}, series, { fields: __spread(series.fields, [
            {
                name: 'LogLevel',
...
function describeTimeRange(range) {
    ...
    if (isDateTime(range.from)) {
        var toMoment = parse(range.to, true);
        return toMoment ? formatDate(range.from) + ' to ' + toMoment.fromNow() : '';
    }
    if (isDateTime(range.to)) {
        var from = parse(range.from, false);
...
exports.setMarkdownOptions = setMarkdownOptions;
...
exports.DataFrameHelper = DataFrameHelper;
...
exports.ScaledVector = ScaledVector;
